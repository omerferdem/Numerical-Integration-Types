...
cubic_result_num_10 = midpoint(f,0,2,10);
cubic_result_num_100 = midpoint(f,0,2,100);
cubic_result_num_1000 = midpoint(f,0,2,1000);
...
exp_result_num_10 = midpoint(g,0,3,10);
exp_result_num_100 = midpoint(g,0,3,100);
exp_result_num_1000 = midpoint(g,0,3,1000);
...
sin_result_num_10 = midpoint(s,0,3,10);
sin_result_num_100 = midpoint(s,0,3,100);
sin_result_num_1000 = midpoint(s,0,3,1000);
...
title('Midpoint Rule Errors');
legend('cubic','exp','sin');
xlabel('Division points');
ylabel('Error ratio');
end

function [result]=midpoint(infunc,low,high,partition)
interval=(high-low)/partition;
result=0;
for i=1:partition
    result=result+interval*infunc((low+interval*(i-1)/2+interval*(i)/2));
end
end